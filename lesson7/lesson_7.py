...
#
# # v1
# users3 = users.intersection(users2)  # перетин множин (що загальне у першої множини з другим) @ порівняння з виявленням однакових елементів
# # v2
# print(users & users2)
# print(users3)
#
...
#
# print(users.issubset(superusers))  # True @порівняння чи є значення в users в superusers
...
# def user_info(name: str = "noname", age: int = 18, hobby: str = "no hobby"): #@ спочатку йдуть обов'язкові, а потім ті що за замовчуванням
#     print(f"Welcome, {name}! Your age: {age} and hobby is {hobby}")
...
#@ якщо вказати значення за дефолтом в функції, то значення буде застосовуватись якщо воно відсутнє
#####
## Усі параметри,
# які розташовуються праворуч від символу *, отримують значення лише на ім'я:
 #@ це значить що можна не вказувати name=
